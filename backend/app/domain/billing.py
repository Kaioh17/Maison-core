...
# `free` is a real $0 recurring price like every other tier, not a special
# case: it goes through the same Checkout Session, so `payment_method_collection
# = 'always'` still forces a card on file even though nothing is due today.
# The retired `starter` price is unmapped, which makes price_to_plan fail
# closed on any stale checkout link still pointing at it.
_PLAN_TO_PRICE = {
    PlanName.FREE.value: _settings.stripe_price_free,
